...
| Workable (`apply.workable.com/<org>/j/<id>/`) | ✅ Tier 1 ATS | 🚫 out of scope | Listing page 1 (contact info) extracts via Tier 2b click-through. **Multi-step page-2+ traversal (CV upload + cover letter + custom questions) is intentionally OUT OF SCOPE** until the auto-submit feature ships — see "Known limitations" below. | 2026-05 |
...
that CTA and reaches the contact-info page. Some Workable postings then
continue over multiple pages (contact-info → CV upload → cover-letter / Q&A
→ submit); some are single-page where every input lives on one screen.

**We deliberately stop at the first page** for both shapes. Going further
would mean either (a) navigating multi-step forms by clicking
"Continue" / "Next" buttons in JS, or (b) clicking "Apply" on
single-page forms — and there's no robust runtime way to tell whether
a given button is a navigation CTA or the final submit. A misclassified
click would cause an unintended submission to the real ATS, which is
strictly worse than missing some fields.
...
where any "Apply" / "Continue" / "Submit" button is allowed to fire.
Until then, Workable postings surface profile-fillable RequirementItems
from page 1 only; users complete the rest manually via the handoff
package.

Practical workaround for the user today: use the "package and bounce"
path (gate-1 `ignore_for_now` for required items) and finish in the
browser. The tailored CV / Cover Letter / answers are still produced
and downloadable.
